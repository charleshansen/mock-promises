(function() {
  ...
  var MockPromiseClass, RealPromiseClass, realThen;
  var MockPromises = function() {
    ...
    this.install = function(ExternalPromiseClass) {
      ...
      if(MockPromiseClass.prototype.then !== fakeThen) {
        realThen = MockPromiseClass.prototype.then;
      }
      MockPromiseClass.prototype.then = fakeThen;
    };

    this.reconstruct = function(ExternalPromiseClass) {
      if(!RealPromiseClass) {
        RealPromiseClass = ExternalPromiseClass;
      }
      ...
      MockPromiseClass.prototype.initialize = function(outerCallback) {
        ...
        var promise = new RealPromiseClass(function(resolve, reject) {
          outerCallback(
            function(value) {
    ...
    this.uninstall = function() {
      if(MockPromiseClass && realThen) {
        MockPromiseClass.prototype.then  = realThen;
      }
      if(RealPromiseClass) {
        MockPromiseClass = RealPromiseClass;
        RealPromiseClass = null;
      }
    };

    this.deconstruct = function() {
      return RealPromiseClass;
    }
    ...
    this.executeForPromise = function(promise){return contractsTracker.executeForPromise(promise);};
    this.valueForPromise = function(promise){return contractsTracker.valueForPromise(promise);};
    this.executeForResolvedPromises = function(){return contractsTracker.executeForResolvedPromises();};
